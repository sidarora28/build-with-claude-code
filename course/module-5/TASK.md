...
- `app/`, `components/`, `lib/` — the dashboard. Five panels: **AGENTS** (specialist roster + LEDs), **FLOW** (per-dispatch timing bars), **PERF** (locked until M6), **BRIEF** (EA's morning brief, typed out live), **TRANSCRIPT** (raw event log).
...
>
> And because we're going to watch four agents move together, today we also turn on something we've kept dormant — the Daily Brain dashboard. By the end of this module, you'll have a live dashboard in your browser showing the coordination as it happens."
...
> ```
>
> After a few seconds, Next.js prints something like:
>
> ```
> ▲ Next.js 14.2.x
...
> ```
>
> **Look for the `Local:` line — that's the URL.** Two easy ways to open it:
>
> - **Click it directly in the terminal.** Most terminals make URLs clickable: **Cmd+click** on Mac, **Ctrl+click** on Windows/Linux. The URL opens in your default browser.
> - **Or copy-paste it.** Triple-click to select the URL line, copy, paste into your browser's address bar, hit Enter.
>
> Tell me 'up' when you see the dashboard in the browser."
...
>
> **Heads-up before you do:** this takes about 30–60 seconds. Four agents are working in parallel — there's no spinner in the terminal, but the dashboard will start animating within a couple of seconds. **If the terminal looks frozen, look at the browser** — that's where you'll see things move first."
...
>
> If you want to try: tell me which specialist feels off, or just say 'tighten notes' and I'll show the smallest possible edit. Or reply 'skip' and we go to the no-chat demo."
...
### Beat 9 — Hands-on: run the orchestrator from a fresh terminal, no chat
...
> - You watched four agents coordinate live, in chat AND as a one-line command from a fresh terminal.
...
| Wants to skip the dashboard | "You can skip it. The orchestrator works fine without the dashboard — it'll write the brief to a file and print to chat. You'll just miss the live visualization. If you'd rather not install Node, reply 'no dashboard' and I'll run the orchestrator straight in chat without the dashboard." |
| `claude -p` not found | Same fix as M3/M4: PATH issue, `which claude`, reopen the terminal. |
| No-chat run completed but dashboard didn't update | "Two checks: (1) is the dashboard's `npm run dev` still running in its terminal? (2) are you watching the right port? Refresh the tab and check the transcript pane — events should appear within a second of the no-chat run finishing." |
...
- [ ] `npm install` completed; `npm run dev` is running; `localhost:3000` is open in the learner's browser; the dashboard is visible (AGENTS + FLOW + PERF + BRIEF + TRANSCRIPT panels, everything idle).
...
- [ ] No-chat run completed from another terminal; dashboard animated identically to the chat run.
