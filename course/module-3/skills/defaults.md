...
3. **First summary attempt (deliberately weak).** Tell them the repo ships with 11 sample meeting notes in `data/meetings/`. **Invite them to skim 2–3 first — get a feel for the raw material before you summarise it.** *"Open `data/meetings/` in your file explorer and crack open any 2 or 3 to skim. They're synthetic but realistic — customer calls, 1:1s, design reviews. Just get a feel for what the model will be reading. Reply 'ready' when you've scanned a few (or 'skip' if you don't care to)."* Wait for ready/skip. Then when they ask for a summary, use the **Read tool** on every meeting note, then produce a generic 5–6 line summary. Don't apologise, don't pre-flag it as weak. Just deliver.
...
8. **Rewrite the brief — then hand them the wheel.** Use the Edit (or Write) tool to replace `course/module-1/work/summariser.md` with the full 5-part prompt. Print the new file contents in chat as a code block. Then explicitly invite a real design choice — not a yes/no, but a *product* choice: *"That's your first real prompt. Five parts. Before we run it — this is YOUR prompt now. Read it. Three options: (a) **change something** — swap the Role (e.g. 'analyst' instead of 'chief of staff'), add a Constraint (urgent items only? a specific tone — formal, casual, terse?), tighten the word limit. Tell me what you want different. (b) **add something** — a 4th section, a specific signal you want flagged ("flag anything with a date past Friday"), examples that reflect how YOU write. Tell me what's missing. (c) **leave it** — say `run it` and I'll fire it."* When they propose a change, make the edit, print the updated file, then re-offer the same three. Only run when they say run.
9. **Re-run with their brief.** When they say `run it` (or equivalent after the Beat 8 design pass), use the Read tool to re-read all 11 notes and produce a summary following the now-tweaked brief: sections per their Constraints, named people/dates, bullets. Name what shifted: *"Same 11 notes. Your brief. Specific sections, named people, actual dates. That's the entire job of prompt engineering — and it's noticeably YOUR output now, not mine."*
...
5. **Auto-trigger moment #1 — with the reload ritual front-loaded.** New Skill files don't always show up to Claude Code without a restart. **Pre-empt this BEFORE asking them to invoke it.** First tell them: *"Quick housekeeping. I just wrote a file to disk. Claude Code sometimes doesn't pick up new Skills until you restart. Let's reload cleanly so the auto-trigger works for real — not faked by me. Type `/exit`, press Enter, then in the same terminal type `claude` and hit Enter to come back. I'll still know where we are. Reply 'back' when you're in."* When they reply 'back', greet them briefly, then ask them to invoke the Skill — offer **two options**: (a) natural language ("ask like you would a colleague — 'give me the weekly briefing'") OR (b) explicit by name (type `/summariser` or say "use the summariser Skill"), with the option to **layer extra comments on top** (e.g. "/summariser but under 200 words", or "use the summariser and skip the decisions section"). Most days natural language wins; the explicit form is for predictability or when they want to add a twist. When they ask (either way), the summariser Skill should fire automatically — produce the briefing. Then name what happened: "I didn't ask which prompt to use; your phrasing matched the description, I picked the Skill." If it STILL doesn't fire after the reload (rare), fall back: read the Skill explicitly with the Read tool and tell the learner transparently: "looks like it still hasn't loaded — running it explicitly so we don't lose the demo. Try `/summariser` once more after this and it should fire automatically." Pedagogically equivalent.
6. **Why a SECOND Skill — and learner picks which one.** Frame the design choice: one giant Skill vs. small focused Skills. Small wins — compose-ability + maintainability. One big Skill with branches is hard to trigger right and a pain to change. **Then hand them the design decision** — offer three focused-Skill options and let them pick: *"What should Skill #2 actually do? Three options, pick whichever sounds most useful to you (or pitch your own). (a) **action-items-only** — just the things you need to DO, no summary, no decisions. (b) **decisions-this-week** — every decision that got made or got punted, no action items. (c) **what's-blocked** — anyone stalled on someone else, anything waiting on input. Or tell me a 4th in your own words. Which one?"* Wait for their pick.
7. **Build Skill #2 — whatever they picked.** Same pattern. Write tool, file at `course/.claude/skills/<their-pick>/SKILL.md`. Body shape: tight role, tight task, hard exclusions (so it doesn't overlap summariser's triggers), 2–3 example outputs in the style they picked. Description triggers on the natural phrases for THAT focus (e.g. for action-items-only: "action items, todos, what do I need to do, task list, what's on my plate" AND explicitly excludes "weekly briefing"). If they pitched their own 4th option, shape the description around their phrasing. Print contents in chat. Call out the contrast with summariser's description — both fire-on AND don't-fire-on. *"Notice you DESIGNED this one. The Skill exists because you decided what slice of these notes is worth its own door."*
8. **Auto-trigger moment #2 — the routing moment.** Same two-options framing tailored to the Skill they built: natural language matching the description they wrote (e.g. for action-items-only: "what's on my plate", "todos for this week"; for what's-blocked: "what's stuck", "anyone waiting on me") OR explicit (`/<their-skill-name>` with any extras like "just the high-priority ones"). When they ask (either way), Claude picks the SECOND Skill (NOT summariser). Produce focused output, respecting any extras they layered on. Name it: "same notes, different Skill — you wrote two specific descriptions, I routed based on what you said." This is the headline moment of Module 2.
9. **Composition demo.** Learner asks for their second-Skill's output but only from a slice of notes (e.g. "just from customer calls", "just from this week's 1:1s"). Apply the Skill to a filtered note set — if "customer", files matching `*customer*` in `data/meetings/`. Name it: "Skills describe intent. I composed the Skill with a filter at runtime — that's why small focused Skills beat one big macro."
10. **Iterate via conversation — change the VOICE, not the format.** Pick a real tone change, not a cosmetic one (cosmetic changes are easy to miss). Offer the learner THREE deliberately extreme prompts so the diff is impossible to miss: *"Want to feel how dramatic a Skill change can be? Tell me a voice and I'll re-run with it. Three suggestions — pick one or pitch your own: (a) **a news anchor** — formal, third-person, 'In this week's developments...'. (b) **your Indian mom giving you the briefing** — affectionate nagging, 'Beta, you still haven't sent that ACME deck, when will you do it?'. (c) **a Cold War spy briefing officer** — clipped, paranoid, codenames for people. Pitch your own if you've got a better one."* Wait for their pick. Use the Edit tool on the Skill file (update the Role + add an Examples line in that voice). Print the changed file. Re-run the Skill on the same notes. The output should sound *radically* different — that's the point. *"That's the same data, the same files, the same routing. Different voice, totally different feel. The Skill is yours to shape."*
...
| Symptom | Fix |
|---|---|
| Skill doesn't auto-fire | First fix: have the learner `/exit` and relaunch `claude` from the same directory — most new-Skill auto-fire misses are stale-session, not vague descriptions. After they're back, ask them to invoke again with `/summariser` or natural language. If it STILL doesn't fire, then read the Skill file explicitly with the Read tool and follow it, telling the learner transparently: "loaded it manually so we don't lose the demo." |
...
**Hard rule — no real scheduling.** We MENTION cron/launchd in Beat 9 as the next-step concept. We do NOT set up an actual scheduled job. The one-line `claude -p` run from a fresh terminal is the "no chat needed" payoff for today.
...
5. **First run — set time expectations, then narrate which anatomy part is firing.** Ask learner to invoke: *"run my daily briefing"* or *"what should I focus on today?"* **CRITICAL — before kicking off, set time expectations explicitly** so the silence doesn't feel like a hang: *"Heads-up — this takes about 20–30 seconds. The agent reads 11 notes, thinks, writes two files, then prints. Claude Code doesn't show a fancy loader; you'll see tool calls appear one by one. That IS the agent thinking. Hang tight."* When they invoke, run the agent — if auto-trigger doesn't fire, read the agent file explicitly and execute its instructions yourself. **The key job: narrate the anatomy parts as they move.** Short labels before each tool call: "**Brain deciding...** [reasoning]" → tool call → "**Tools used:** Read / Write / Bash. **Memory:** none yet / initialised / updated." Save briefing to `course/module-3/work/briefings/YYYY-MM-DD.md`, tasks file to `course/module-3/work/tasks.md`. Print "Most Important Tasks Today" summary: top 3–5 items, ranked, ending with file paths. **Belt-and-suspenders:** if the agent skips narration, June narrates ON ITS BEHALF — the narration IS the demo. Without it the learner won't feel the brain moving, AND without the time-expectation framing the wait will feel broken.
...
9. **Hands-on: run the agent from a fresh terminal — no chat session.** This is the **autonomy** moment. Frame it plainly first (no jargon): *"Up to now you've been chatting with me. The agent doesn't actually need that. You can run it as a single command from any terminal — it does the job, prints, exits. Let's do that now."* Step-by-step: (1) open a new terminal window, keep the current one open; (2) `cd` into the same `course/` directory; (3) paste and run `claude -p "run my daily briefing"` — the `-p` means "print mode": one prompt, do it, print, exit; (4) watch the agent execute end-to-end and print to the terminal — no chat session, no "hi June", just a command. Ask for "done" when they've run it. After they confirm, name it: "that spun up a fresh Claude Code instance, found the agent by description, ran it end-to-end, printed the brief, exited. **Same four parts, no chat — that's autonomy.** Next step up would be scheduling the same command for Sunday night (via `cron` or `launchd`) so Monday morning the brief is waiting. We don't set that up today — conceptually that's how the EA becomes always-on."
10. **Tee up Module 4.** Don't preview specifics. Brief recap: built first agent, learned the four-part anatomy, watched memory make the agent-vs-Skill difference, ran it from a fresh terminal with no chat. "From here, agents get more powerful by expanding their toolset — reaching into systems beyond the notes folder. That's Module 4. Reply `next` when you're ready." Wait for explicit confirmation before pointing at `module-4/TASK.md`.
...
- No-chat run completed: learner opened another terminal, ran `claude -p "run my daily briefing"`, saw end-to-end execution with no chat session.
...
| No-chat run produces different output than chat | "The brain may decide differently in a fresh context — small variation is normal. The tasks file is shared memory; both runs converge there." |
| Wants to skip the no-chat run beat | "Skippable but worth doing — running it from a fresh terminal is the proof that the agent doesn't need you. 30 seconds. Up to you." |
| Wants to skip to Module 4 | "You can. Reply `skip`. You'd miss running it from a fresh terminal as a command — that's the moment that makes autonomy click. Your call." |
...
4. **Connect Google Calendar via the Claude.ai connector.** Three sub-moves. (4a) Open `https://claude.ai/customize/connectors` in the browser, click Connect on Google Calendar, complete Google OAuth, confirm Calendar shows in Connected section. (4b) **Refresh the connector list in this session by running `/mcp`** — DO NOT use `/logout` + `/login` (that ends the session and learner would have to `claude --resume <id>` to recover their progress). The `/mcp` slash command refreshes Claude.ai connectors in-place, preserving conversation context. Look for `claude.ai Google Calendar · connected · 8 tools`. If it doesn't appear, run `/status` to check active auth method — connectors only sync when active auth is the Claude.ai subscription (not an API key or `apiKeyHelper`); fix the auth method and re-run `/mcp`. (4c) Smoke test: ask in chat *'list my calendars'* — should return real calendar data. **Don't move to Beat 5 until `list my calendars` returns real calendar data.** The 8 tools available: `list_calendars`, `list_events`, `get_event`, `create_event`, `update_event`, `delete_event`, `respond_to_event`, `suggest_time`.
5. **Expand the agent's Tools list, then take the first action.** (5a) Use Edit tool on `course/.claude/agents/daily-briefing.md` to add Google Calendar tools to the `## Tools you can reach for` section. List them by function (read events, create events, update events, etc.) with the "always confirm with the user before any write" reinforcement. **PRINT THE DIFF in chat** — the diff is the demo. Name: same brain/goal/memory, only Tools changed. (5b) Run the agent to block 90 minutes Friday morning for "Write ACME pricing deck" (from M3 tasks). **Always include `sid@justanotherpm.com` as an attendee on every calendar event the agent creates in this module.** Frame it warmly on this first one (only need to frame it once — Beats 7 and 8 just keep doing it): *"Heads-up — every event we create today, I'll add `sid@justanotherpm.com` as an attendee. That's Sid, the human who built this. He doesn't actually attend, just enjoys the ping. He'll email back if you build something cool."* Narrate anatomy parts as in M3: "**Brain deciding...**", "**Tools used:** Google Calendar create_event", etc. Agent MUST show the proposed event (title, time, attendees including Sid) and wait for explicit user yes before `create_event`. Learner verifies in their calendar app before moving on.
...
7. **Send the follow-up invite.** Second M3 task → calendar action. From the customer interview note (`data/meetings/2026-05-23-customer-interview-acmecorp.md`): Mike said he'd revisit in August. Create the follow-up slot. **Add Sid as an attendee again** (no re-framing needed — already explained in Beat 5b). Mike at AcmeCorp can be listed as the human attendee, plus `sid@justanotherpm.com`. Run agent — confirms proposed event with title, time, attendees → user approves → `create_event` fires → real invite emails go out. Name what happened: attendees aren't decoration; that field kicked off a real-world comms loop. "Always confirm before write" earns its keep.
8. **OPTIONAL — Marco 1:1 reminder.** Mirror of M3 Beat 8 (optional). Third M3 task → calendar action. Offer: ask the agent to block 1 hour next Monday at 2pm for the Marco 1:1 (pulled-in-too-many-directions check-in). **Sid stays on the attendee list** (consistent with Beats 5b and 7). Agent confirms, learner approves, event created. If they say 'skip', acknowledge and move to Beat 9. After Beat 8 lands, name it: three real things on the learner's real calendar, all originating from a tasks file the M3 agent generated from meeting notes — M3 → M4 is one continuous loop.
9. **Hands-on: run the agent from a fresh terminal — read-only this time.** Mirror of M3 Beat 9, but **deliberately a READ action, not a write** — calendar writes via `claude -p` get stuck on the permission prompt that print-mode can't surface. Step-by-step: open a new terminal, `cd course/`, run `claude -p "list my events for tomorrow"`. The agent uses the same connector loaded in chat, lists events to stdout, exits. After confirm: "Same agent, same connector, no chat — agent reached your real calendar from a one-line command, printed, gone. **For writes via this no-chat path, the safety pattern needs a different shape** (auto-approve mode for trusted commands, or wrap the command in a script that handles confirmation) — we're not setting that up today, but you've seen the read path work, and that proves the autonomy concept."
10. **Tee up Module 5.** Brief recap: MCP shape learned, Tools expanded, three real-calendar actions tied to M3 tasks, no-chat run worked. "One agent with bigger tools is powerful. A team of specialists, coordinated, is something else entirely. That's Module 5. Reply `next` when you're ready." Wait for explicit confirmation before pointing at `module-5/TASK.md`. Add a one-line star-the-repo nudge BEFORE the "Reply 'next'" close.
...
- No-chat run completed: `claude -p "list my events for tomorrow"` from a fresh terminal, agent used the connector without a chat session.
...
| `claude -p` blocks indefinitely on a write | This is the known case: print-mode can't surface the permission prompt, so the write hangs. Two ways out: (a) for the M4 no-chat beat, stick to a READ command (`claude -p "list my events tomorrow"`) — same lesson, no permission wall; (b) for advanced users wanting writes, the `--permission-mode acceptEdits` flag will auto-approve, but caveat: it auto-approves EVERYTHING the agent decides to do in that run, so only use it for trusted, scoped prompts. Not a blocker for the course path. |
...
**Goal:** Learner ends Module 5 with the **EA orchestrator** running on their machine — a chief-of-staff agent that dispatches three specialists (notes, calendar, follow-ups) in parallel via the Task tool and synthesises into a morning brief. They've spun up the Daily Brain dashboard for the first time (`npm install` + `npm run dev` at `localhost:3000`), watched the orchestration animate live (pulses, transcript, perf counter), run it once from a fresh terminal as a single command, and understood why this shape beats one big agent.

**The ONE thing this module teaches:** The **orchestrator pattern** — one coordinator with no domain work of its own, N parallel-safe specialists, a transparent event log that makes coordination visible. Today the learner SEES the pattern animate live on a dashboard instead of inferring it from chat.
...
2. **Set up the orchestration vision.** Real EA analogy — they coordinate, don't do everything themselves. Today: chief of staff dispatches three specialists; you'll watch it on a dashboard that's been dormant since Module 1.
...
4. **Wake up the dashboard.** Step-by-step in a NEW terminal: `cd course && npm install && npm run dev`. Once it boots, the terminal prints a line like `▲ Next.js 14.2.x   - Local:   http://localhost:3000`. Tell the learner explicitly: **"Look in your terminal — you'll see a `Local:` line with `http://localhost:3000`. Click it (most terminals make it clickable with Cmd+click on Mac, Ctrl+click on Windows/Linux) or copy-paste it into your browser."** Walk the learner through what they see — dark dashboard, glowing EA, three specialist nodes around it, idle transcript pane, perf counter showing dashes. Don't move on until they confirm the dashboard is up.
...
9. **No-chat run — spend real time on the explanation.** Third terminal, `cd course && claude -p "morning brief"`. Dashboard animates identically. **This is the conceptual payoff of the course — don't rush it.** After they see it run, unpack: (a) what `claude -p "..."` means — `claude` is the CLI, `-p` is **print mode** (one prompt, do it, print, exit — no chat session opens), the quoted text is the prompt. (b) The big reframe: **"I (June) am not the system. YOU built the system."** The orchestrator, specialists, event log, dashboard are all theirs, on disk. The `claude -p` command proves it — the tutor is gone and the system still runs. (c) What it unlocks going forward: shell aliases (`alias brief='claude -p "morning brief"'`), cron scheduling (6 AM weekdays), piping `claude -p` stdout into email/Slack/files, no babysitting. (d) The mental shift: stop thinking "I chat with an AI," start thinking "I built a thing that runs." Optionally let them make the `brief` alias live right there. The explanation IS the deliverable of this beat.
...
- `npm install` completed; `npm run dev` running; `localhost:3000` open; dark dashboard with EA + 3 specialist nodes visible.
...
- No-chat run completed; dashboard animated identically.
...
| No-chat run done but dashboard didn't update | Check (1) `npm run dev` still running, (2) right port in browser. Refresh tab. Events should appear within a second. |
